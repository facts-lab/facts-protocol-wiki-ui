...
export class ArweaveService {
  ...
  getNetworkInfo(): Observable<NetworkInfoInterface> {
    console.log('NETWORK', this.arweave);
  ...
  getAccount(method: string): Observable<string> {
    ...
    return obs.pipe(catchError(this.errorHandler));
  }
  ...
  */
  ...
  private async _uploadFileToArweave(
    ...
    externalProgressObj?:
      | { completed: string; uploaded: string; total: string }
      | undefined
      | null
  ): Promise<Transaction | { id: string; type: string } | DispatchResult> {
    ...
    if (loginMethod === 'arconnect' && !disableDispatch) {
      ...
      if (+transaction.data_size > this.dataSizeLimitDispatch) {
        throw new Error(
          `Dispatch is not available for data size > ${this.dataSizeLimitDispatch} bytes.`
        );
      }
  ...
  async sendFee(_contractState: any, _fee: string, jwk: any): Promise<any> {
    ...
    const tx = await this.arweave.createTransaction(
      {
        target: holder,
        quantity: this.arweave.ar.arToWinston(_fee),
      },
      jwk
    );
  ...
  getDataAsStringObs(txId: string): Observable<string> {
    return this.getDataAsString(txId).pipe(
      catchError((error) => {
        ...
        return from(
          this.arweave.transactions.getData(txId, {
            decode: true,
            string: true,
          })
        ).pipe(
  ...
  async getTxContent(txId: string) {
    ...
    if (error) {
      try {
        const tmpContent = await this.arweave.transactions.getData(txId, {
          decode: true,
          string: true,
        });
  ...
  uploadFileToArweave(
    ...
    externalProgressObj?:
      ...
      | undefined
      | null
  ): Observable<Transaction | { id: string; type: string } | DispatchResult> {
    return from(
      this._uploadFileToArweave(
        fileBin,
        contentType,
        key,
        tags,
  ...
  public async getUserSigner(
    method: string
  ): Promise<InjectedArweaveSigner | null> {
    if (method === 'arweavewebwallet') {
      const userSigner = new InjectedArweaveSigner(this.arweaveWebWallet);
      return userSigner;
